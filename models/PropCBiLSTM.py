...
def get_input_nn(input_shape, dropout, conv_num_hidden, conv_kernel_size, max_pool,
                 pool_size, lstm_num_units, lstm_bidirectional, lstm_before_cnn):
    nn = Sequential()
    if lstm_before_cnn:
        if lstm_bidirectional:
            lstm = Bidirectional(
                LSTM(units=lstm_num_units, dropout=dropout, recurrent_dropout=dropout, return_sequences=True),
        ...
        nn.add(get_1d_pool(pool_size=pool_size, max_pool=max_pool))
        ...
        if lstm_bidirectional:
            lstm = Bidirectional(
                LSTM(units=lstm_num_units, dropout=dropout, recurrent_dropout=dropout),
                input_shape=input_shape
...
class PropCBiLSTM(object):

    def __init__(self, input_shape, dropout=0.5, conv_num_hidden=256, conv_kernel_size=3, max_pool=True,
                 pool_size=2, lstm_num_units=32, lstm_bidirectional=False, lstm_before_cnn=False,
                 dense_num_hidden=1024, lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-8):
        claim_cnn = get_input_nn(
            input_shape=input_shape,
            dropout=dropout,
            ...
            lstm_before_cnn=lstm_before_cnn
        )
        body_cnn = get_input_nn(
            input_shape=input_shape,
            dropout=dropout,
            conv_num_hidden=conv_num_hidden,
            conv_kernel_size=conv_kernel_size,
    ...
    def train(self, titles, bodies, labels, epochs, seq_len, num_classes=3,
        ...
        if logs_name is not None:
            callbacks.append(TensorBoard(log_dir=get_tb_logdir(f"PropCBiLSTM_{logs_name}")))
...
if __name__ == '__main__':
    ...
    history = model.train(
        ...
        logs_name=f"{NUM_CONV_HIDDEN}CONV-{NUM_DENSE_HIDDEN}DENSE-" +
                  f"{Bidirectional}BIDIR){NUM_LSTM_UNITS}LSTM-" +
                  f"{DROPOUT}DOUT-{USE_MAXPOOL}MXPL-{NUM_EPOCHS}EPOCHS"
    )
